...
const RentCard = ({ rent }) => {
  const { _id, city, state, photo, price, featured, reviews, bed, bath, propertyType } = rent;
    ...
    <div className="rent__card">
      <Card>
        <div className="rent__img">
          ...
          {/* src={photo01}    src={`/rent-images/${photo01}`} */}
        ...
        <CardBody>
          ...
          <h5 className="rent__title">
            <Link to={`/rents/${_id}`}>{bed} BHK House For rent in {city}, {state}</Link>
          </h5>
          <div className="d-flex align-items-center justify-content-between">
            <span className="d-flex align-items-center gap-1">
            <img className="bed__icon" src={BedImg} alt="bed-img" /> {bed}
            <img className="bath__icon" src={BathImg} alt="bed-img" /> {bath}
            <img className="bath__icon" src={BuildingImg} alt="bed-img" /> {propertyType}
            </span>
          </div>
